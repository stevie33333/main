async function setupHotkeys(accounts, override_url) {
    var hotkeys = [
        ...
        {"keys" : ["shift", "alt", "ctrl", "keya"], "f" : () => buttonClickBuySellJoin(false)},
        ...
        {"keys" : ["ctrl", "numpad1"], "f" : () => setAccount(accounts[S1APR184094639])},
        {"keys" : ["ctrl", "numpad2"], "f" : () => setAccount(accounts[S1MAY104385237])},
        {"keys" : ["ctrl", "numpad3"], "f" : () => setAccount(accounts[S1MAY104397921])},
...
{
    console.log('spinbutton ' + quantity + ' hotkey pressed');
    setQuantityCommon(quantity);
}

async function setContract(contract, delayMilliseconds)
{
    var input_div = [...document.querySelectorAll('div[class^=MuiInputBase-root')].filter(d => d.innerText.toLowerCase().startsWith('spinbutton'))[0]
    if(input_div == null)
    {
